...
class Buku extends BaseController
{
    ...
    public function store()
    {
        ...
        // =========================
        // INPUT (SESUAI FORM DATALIST)
        // =========================
        $kategori = $this->request->getPost('nama_kategori');
        $penulis  = $this->request->getPost('nama_penulis');
        $penerbit = $this->request->getPost('nama_penerbit');
        $rak      = $this->request->getPost('nama_rak');
    
        // =========================
        // SIMPAN / CARI KATEGORI
        // =========================
        $dataKategori = $kategoriModel->where('nama_kategori', $kategori)->first();
        if (!$dataKategori) {
            $kategoriModel->insert(['nama_kategori' => $kategori]);
        ...
        // PENULIS
        // =========================
        $dataPenulis = $penulisModel->where('nama_penulis', $penulis)->first();
        if (!$dataPenulis) {
            $penulisModel->insert(['nama_penulis' => $penulis]);
            $id_penulis = $penulisModel->insertID();
        } else {
            $id_penulis = $dataPenulis['id_penulis'];
        }
    
        // =========================
        // PENERBIT
        // =========================
        $dataPenerbit = $penerbitModel->where('nama_penerbit', $penerbit)->first();
        if (!$dataPenerbit) {
        ...
        $id_rak = null;
        if ($rak) {
            $dataRak = $rakModel->where('nama_rak', $rak)->first();
            if (!$dataRak) {
                $rakModel->insert([
                    'nama_rak' => $rak,
                    'lokasi' => ''
